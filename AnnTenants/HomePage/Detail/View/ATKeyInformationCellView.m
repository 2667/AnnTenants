...
- (void)bindModel:(ATShareHouseJsonModel *) shareHouseJosnM {
    self.rentType.text = shareHouseJosnM.rentType;
    self.price.text = shareHouseJosnM.rent;
    
    if ([@"整租" rangeOfString:shareHouseJosnM.rentType].length > 0) {
        self.houseType.text = @"整套";
    }else {
        self.houseType.text = @"合租";
    }
    
    self.area.text = [NSString stringWithFormat:@"%@㎡",shareHouseJosnM.area];
    self.direction.text = shareHouseJosnM.orientation;
    self.onHireDuration.text = @"可短租";
    self.paymentWays.text = shareHouseJosnM.paymentWays;
    self.decorated.text = shareHouseJosnM.decorated;
    self.singleToilet.text= @"独卫";
}
