...
- (void)bindModel:(ATShareHouseJsonModel *) shareHouseJsonM {
    self.area.text = [NSString stringWithFormat:@"%@㎡", shareHouseJsonM.area];
    self.floor.text = shareHouseJsonM.floor;
    self.number.text = shareHouseJsonM.houseingResourceID;
    
    NSDate * myDate=[NSDate dateWithTimeIntervalSince1970:[shareHouseJsonM.publishTime doubleValue]];
    //设置时间格式
    ...
    self.houseType.text = shareHouseJsonM.houseType;
    self.decoration.text = shareHouseJsonM.decorated;
}
