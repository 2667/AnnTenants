...
- (void)bindModel:(ATShareHouseJsonModel *) shareHouseJsonM {
    [self layoutOfSubviewsWithFacilities:shareHouseJsonM.houseConfiguration];
}

- (void)layoutOfSubviewsWithFacilities:(NSString *) facilities {
    NSArray *array = [facilities componentsSeparatedByString:@","];
    for (int i = 0; i < array.count; i ++) {
        UIView *view = [self createFacility:array index:i];
        [self.contentView addSubview:view];
    }
