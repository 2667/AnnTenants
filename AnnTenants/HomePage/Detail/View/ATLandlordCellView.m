...
- (void)bindModel:(ATShareHouseJsonModel *)shareHouseJsonM {
    self.price.text = [NSString stringWithFormat:@"%@￥",shareHouseJsonM.rent];
    
    ATMyHeadModel *myHeadM = [ATHouseResourceViewModel userOrLandlordInfoByID:shareHouseJsonM.userId];
    self.nickName.text = myHeadM.nickName;
    self.headImage.image = [ATTools readPictureFromAbsolutePath:myHeadM.avatarPath];
    self.callUp.text = myHeadM.phoneNumber;
}
