...
//    设置单元格开始在tableView显示的位置
    ...
    NSArray *pictures = [NSArray arrayWithArray:self.shareHouseJsonModel.housePictures];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        NSArray *hc = [self.shareHouseJsonModel.houseConfiguration componentsSeparatedByString:@","];
...
- (void) collectAction {
    if (![ATTools isBlankString:ATUserManager.shareUser.userId]) { //判断用户是否注册或者登录
        if (!_isCollect) {
            _isCollect = YES;
            ATCollectModel *collectM = [ATCollectModel initWithUser:self.shareHouseJsonModel.userId houseID:self.shareHouseJsonModel.houseingResourceID isCollect:@"已收藏"];
            [ATSQLiteManager isInsertDataForCollectResource:collectM];
            [UIView animateWithDuration:2.0 //动画持续时间
                             animations:^{
                                 [self.collect setTitle:@"已收藏" forState:UIControlStateNormal];
                                 [self.collect setImage:[UIImage imageNamed:@"collect1"] forState:UIControlStateNormal];
                             }];
        }else {
            _isCollect = FALSE;
            [ATSQLiteManager deleteSQLTableForCondition:[NSString stringWithFormat:@"houseResourceID = '%@'", self.shareHouseJsonModel.houseingResourceID] withTableName:@"collectHouseResource"];
            [UIView animateWithDuration:2.0  //动画持续时间
                             animations:^{
                                 [self.collect setTitle:@"收藏" forState:UIControlStateNormal];
                                 [self.collect setImage:[UIImage imageNamed:@"unCollect1"] forState:UIControlStateNormal];
                             }];
        }
    }else {
...
- (void) reservationAction {
    if (![ATTools isBlankString:ATUserManager.shareUser.userId]) {
        ATReservationController *reservation = [ATReservationController new];
        reservation.shareHouseJsonM = self.shareHouseJsonModel;
